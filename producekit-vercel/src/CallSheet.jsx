...
/* ── wider time input — no icon overlap ──────────────────── */
...
const CallSheetModule = ({ project, setProject }) => {
  ...
  useEffect(() => {
    ...
    persistCS({ header: {
      ...
      transportMin: 0, sunrise: "", sunset: "", weather: "",
  ...
  /* ── cascading implicit values ─────────────────────────── */
  const shiftStart = day?.callTime || "06:00";
  const shiftEnd = day?.wrapTime || "18:00";
  const transportMin = h.transportMin || 0;
  const callOnSet = h.callOnSet || (transportMin > 0 ? addMin(shiftStart, transportMin) : shiftStart);
  const firstTake = h.firstTake || callOnSet;
  const estWrap = h.estWrap || (transportMin > 0 ? subMin(shiftEnd, transportMin) : shiftEnd);
  ...
  /* ── REFRESH: recalculate all scenes + breaks + crew ───── */
  const refreshAll = () => {
    ...
    // Reset crew calls to callOnSet (clear manual overrides)
    const resetCrew = {};
    crew.forEach(c => { resetCrew[c.id] = { call: callOnSet }; });

    // Reset cast: onSet = first scene start, cascade costume/makeup
    const resetCast = {};
    dayCast.forEach(c => {
      const firstStart = getFirstSceneStartFromUpdated(c.id, updatedStrips, ordered);
  ...
  // Helper to get first scene start from updated strips
  const getFirstSceneStartFromUpdated = (castId, updatedStrips, ordered) => {
    for (const sid of ordered) {
      const s = updatedStrips.find(x => x.id === sid);
      if (s && (s.cast || []).includes(castId) && s.startTime) return s.startTime;
    }
    return "";
  };
  ...
  const addBreak = (type) => {
    ...
    persistCS({ breaks: [...csBreaks, {
      ...
      duration: bt.defaultDur, afterScene: mid || "",
      startTime: "", endTime: "",
    }]});
  };
  ...
  if (!day) return <div>
    ...
    <div style={{background:"#1a1d23",border:"1px dashed #2a2d35",borderRadius:10,padding:30,textAlign:"center",color:"#555",fontSize:13}}>No shooting days. Add shoot days in Calendar.</div>
  </div>;
    ...
    <div style={{background:"#1a1d23",border:"1px solid #2a2d35",borderRadius:10,overflow:"hidden"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:0}}>
          ...
          <div style={{padding:"10px 16px",borderRight:"1px solid #2a2d35"}}>
            ...
            <HR label="Transport">
              <div style={{display:"flex",alignItems:"center",gap:4}}>
                <input type="number" min="0" step="5" value={transportMin} onChange={e => updateHeader("transportMin", Number(e.target.value))}
                  style={{background:"#12141a",border:"1px solid #2a2d35",borderRadius:4,color:"#ccc",fontSize:12,fontWeight:700,padding:"4px 8px",width:56,fontFamily:"inherit",outline:"none",textAlign:"center"}} />
                <span style={{fontSize:10,color:"#666"}}>min</span>
              </div>
            </HR>
            <HR label="Call on set"><TI value={callOnSet} onChange={v => updateHeader("callOnSet", v)} color="#E8C94A" bold /></HR>
            <HR label="First take"><TI value={firstTake} onChange={v => updateHeader("firstTake", v)} color="#22c55e" bold /></HR>
            <HR label="Est. wrap"><TI value={estWrap} onChange={v => updateHeader("estWrap", v)} color="#ef4444" bold /></HR>
          ...
          <div style={{padding:"10px 16px",borderRight:"1px solid #2a2d35"}}>
            ...
            <div style={{marginBottom:8}}>
              ...
              <select value={h.basecampId || "manual"} onChange={e => { if (e.target.value === "manual") updateHeader("basecampId", ""); else updateHeader("basecampId", e.target.value); }}
            ...
            <div>
              ...
              <AddressInput value={h.muaCostumeAddr || ""} onChange={v => updateHeader("muaCostumeAddr", v)} placeholder="Makeup & costumes..." style={{fontSize:10,padding:"4px 6px"}} />
            </div>
          </div>
          ...
          <div style={{padding:"10px 16px"}}>
            ...
            {HEADER_ROLES.map(({ key, label }) => {
              ...
              return <div key={key} style={{display:"flex",alignItems:"center",gap:4,padding:"2px 0"}}>
                ...
                <input value={contact.name || ""} onChange={e => updateContact(key, "name", e.target.value)} placeholder="Name"
                ...
                <input value={contact.phone || ""} onChange={e => updateContact(key, "phone", e.target.value)} placeholder="Phone"
      ...
      <div style={{padding:"10px 20px 20px"}}>
        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",minWidth:700}}>
            <thead><tr style={{borderBottom:"1px solid #333"}}>
              {["Sc","From","To","Synopsis","Cast","Type","Pgs","Location"].map(hd =>
            ...
            <tbody>
              {sceneOrder.map(item => {
                if (item.type === "scene") {
                  ...
                  return <tr key={s.id} style={{borderBottom:"1px solid #1e2028"}}>
                    ...
                    <td style={{...tdS,width:90}}>
                      <TI value={s.startTime || ""} onChange={v => setProject(prev => ({...prev, strips: prev.strips.map(x => x.id === s.id ? {...x, startTime: v} : x)}))} color="#3b82f6" />
                    </td>
                    <td style={{...tdS,width:90}}>
                      <TI value={s.endTime || ""} onChange={v => setProject(prev => ({...prev, strips: prev.strips.map(x => x.id === s.id ? {...x, endTime: v} : x)}))} color="#3b82f6" />
                    </td>
                    <td style={{...tdS,color:"#aaa",maxWidth:220}}>{s.synopsis}</td>
                ...
                return <tr key={b.id} style={{borderBottom:"1px solid #1e2028",background:bt.color + "08"}}>
                  ...
                  <td style={{...tdS,width:90}}><TI value={b.startTime || ""} onChange={v => updateBreak(b.id, "startTime", v)} color={bt.color} /></td>
                  <td style={{...tdS,width:90}}><TI value={b.endTime || ""} onChange={v => updateBreak(b.id, "endTime", v)} color={bt.color} /></td>
            ...
            <tbody>
              {sortedCast.map(c => {
                ...
                return <tr key={c.id} style={{borderBottom:"1px solid #1e2028"}}>
                  ...
                  <td style={{...tdS,width:90}}><TI value={csData.costume} onChange={v=>updateCastCS(c.id,"costume",v)} /></td>
                  <td style={{...tdS,width:90}}><TI value={csData.makeup} onChange={v=>updateCastCS(c.id,"makeup",v)} /></td>
                  <td style={{...tdS,width:90}}><TI value={csData.onSet} onChange={v=>updateCastCS(c.id,"onSet",v)} color="#E8C94A" /></td>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"0 10px"}}>
          {orderedCrew.map((c, i) => {
            ...
            return <div key={c.id} style={{display:"flex",alignItems:"center",gap:4,padding:"2px 0",fontSize:10,borderTop:showDivider?"1px solid #2a2d35":"none"}}>
              ...
              <TI value={crewCall} onChange={v=>updateCrewCS(c.id,"call",v)} color={early ? "#ef4444" : "#888"} />
            </div>;
          })}
        </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
              ...
              <div style={{display:"flex",gap:3,flexShrink:0}}>
                {dayStrips.slice(0,6).map(s => (
                  <button key={s.id} onClick={() => {
                    const cur = note.text || "";
                    const sep = cur && !cur.endsWith(", ") && cur.length > 0 ? ", " : "";
                    updateNote(note.id, "text", cur + sep + `Sc${s.scene}: `);
                  }} style={{background:"#1a1d23",border:"1px solid #2a2d35",borderRadius:3,color:"#888",fontSize:8,padding:"2px 4px",cursor:"pointer",fontFamily:"inherit",lineHeight:1}}>
                    {s.scene}
                  </button>
                ))}
              </div>
              <input value={note.text || ""} onChange={e=>updateNote(note.id,"text",e.target.value)}
                placeholder={`Sc${dayStrips[0]?.scene || "1"}: props, Sc${dayStrips[1]?.scene || "2"}: set dressing...`}
          ...
          <button onClick={addNote} style={{display:"flex",alignItems:"center",gap:5,padding:"8px 12px",background:"transparent",border:"1px dashed #2a2d35",borderRadius:6,color:"#666",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit",width:"fit-content"}}>
            <I.Plus/> Add Note
          </button>
        </div>
