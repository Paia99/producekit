...
export const I = {
  ...
  Scenes: () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 8V4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v16a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-4"/><polyline points="10 9 15 12 10 15"/></svg>,
...
// ─── SHARED UI ───────────────────────────────────────────────
export const StatusBadge = ({ status }) => { const c = { confirmed:"#22c55e",available:"#3b82f6",imported:"#a855f7",hold:"#f59e0b",unavailable:"#ef4444",draft:"#888",dispatched:"#3b82f6" }; return <span style={{display:"inline-flex",alignItems:"center",gap:4,fontSize:11,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.05em",color:c[status]||"#888",background:(c[status]||"#888")+"18",padding:"2px 8px",borderRadius:4}}><span style={{width:6,height:6,borderRadius:"50%",background:c[status]||"#888"}}/>{status}</span>; };
export const TrafficBadge = ({ note }) => { if(!note)return null; const c = {"Clear roads":"#22c55e","Light traffic":"#22c55e","Moderate traffic":"#f59e0b","Heavy traffic":"#ef4444"}; return <span style={{fontSize:10,fontWeight:600,color:c[note]||"#888",background:(c[note]||"#888")+"15",padding:"2px 6px",borderRadius:3}}>{note}</span>; };
export const Modal = ({ title, onClose, children, width = 560 }) => (<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,backdropFilter:"blur(4px)"}} onClick={onClose}><div onClick={e=>e.stopPropagation()} style={{background:"#1a1d23",border:"1px solid #2a2d35",borderRadius:12,width,maxWidth:"95vw",maxHeight:"85vh",overflow:"auto",boxShadow:"0 25px 60px rgba(0,0,0,0.5)"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 20px",borderBottom:"1px solid #2a2d35",position:"sticky",top:0,background:"#1a1d23",zIndex:1}}><h3 style={{margin:0,fontSize:16,fontWeight:700,color:"#f0f0f0"}}>{title}</h3><button onClick={onClose} style={{background:"none",border:"none",color:"#888",cursor:"pointer",padding:4}}><I.X/></button></div><div style={{padding:20}}>{children}</div></div></div>);
export const IS = {width:"100%",background:"#12141a",border:"1px solid #2a2d35",borderRadius:6,padding:"8px 12px",color:"#e0e0e0",fontSize:13,outline:"none",boxSizing:"border-box",fontFamily:"inherit"};
export const LS = {display:"block",fontSize:11,fontWeight:600,color:"#888",textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:4};
export const BP = {background:"#E8C94A",color:"#111",border:"none",borderRadius:6,padding:"8px 20px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit"};
export const BS = {background:"#2a2d35",color:"#ccc",border:"1px solid #3a3d45",borderRadius:6,padding:"8px 16px",fontWeight:600,fontSize:13,cursor:"pointer",fontFamily:"inherit"};
export const BD = {...BS,color:"#ef4444",borderColor:"#ef444433"};
