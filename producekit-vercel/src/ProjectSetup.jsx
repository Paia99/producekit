...
const ProjectSetup = ({ projects, setProjects, activeId, setActiveId }) => {
  ...
  const openEdit=(p)=>{setForm({id:p.id,name:p.name,production:p.production,shootingDays:p.shootingDays||20,idec:p.idec||"",pagesPerMinute:p.pagesPerMinute||60,keyRoles:{...(p.keyRoles||{})}});setEditModal("edit");};
  const openNew=()=>{setForm({name:"New Production",production:"",shootingDays:20,idec:"",pagesPerMinute:60,keyRoles:{}});setEditModal("new");};
  const save=()=>{
    if(editModal==="new"){const np=defaultProject();np.name=form.name;np.production=form.production;np.shootingDays=form.shootingDays;np.idec=form.idec;np.pagesPerMinute=form.pagesPerMinute;np.keyRoles=form.keyRoles;np.crew=[];np.cast=[];np.strips=[];np.days=[];np.vehicles=[];np.routes=[];np.locations=[];np.schedule={};setProjects(p=>[...p,np]);setActiveId(np.id);}
    else{setProjects(p=>p.map(pr=>pr.id===form.id?{...pr,name:form.name,production:form.production,shootingDays:form.shootingDays,idec:form.idec,pagesPerMinute:form.pagesPerMinute,keyRoles:form.keyRoles}:pr));}
    setEditModal(null);
  };
  return <div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:12}}>
      {projects.map(p=><div key={p.id} style={{background:"#1a1d23",border:`1px solid ${p.id===activeId?"#E8C94A55":"#2a2d35"}`,borderRadius:10,padding:16}}>
        ...
        <div style={{fontSize:11,color:"#666",marginBottom:8}}>{p.idec&&<span style={{color:"#888",marginRight:8}}>IDEC: {p.idec}</span>}{p.crew?.length||0} crew · {p.cast?.length||0} cast · {p.days?.length||0}/{p.shootingDays||"?"} days · {p.strips?.length||0} scenes · {p.pagesPerMinute||60}min/pg</div>
    ...
    {editModal&&<Modal title={editModal==="new"?"New Project":"Edit Project"} onClose={()=>setEditModal(null)}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        ...
        <div><label style={LS}>Minutes per Page</label>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <input type="number" min="10" max="240" step="5" value={form.pagesPerMinute||60} onChange={e=>setForm({...form,pagesPerMinute:Number(e.target.value)})} style={{...IS,width:80}}/>
            <span style={{fontSize:11,color:"#888"}}>min/page (default 60)</span>
          </div>
        </div>
      </div>
