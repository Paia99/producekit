  ...
  return <div>
    ...
    {(editModal==="nA"||editModal==="eA")&&<Modal title={editModal==="nA"?"Add Cast":`Edit — ${form.name}`} onClose={()=>setEditModal(null)}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        ...
        <div style={{gridColumn:"1/-1"}}><label style={LS}>Address</label><input value={form.address||""} onChange={e=>setForm({...form,address:e.target.value})} style={IS}/></div>
        <div style={{gridColumn:"1/-1"}}><label style={LS}>Hotel</label><input value={form.hotel||""} onChange={e=>setForm({...form,hotel:e.target.value})} style={IS}/></div>
    ...
    {editRoute&&<Modal title={editRoute==="new"?"Create Route":"Edit Route"} onClose={()=>setEditRoute(null)} width={640}>
      ...
      <div style={{marginBottom:12}}><label style={LS}>Stops</label>
        ...
        <div key={i} style={{display:"flex",gap:8,alignItems:"center",marginBottom:6,padding:8,background:"#22c55e08",borderRadius:6}}>
          ...
          <select value={s.locationId||""} onChange={e=>{const loc=locations.find(l=>l.id===e.target.value);upSt(i,"locationId",e.target.value);if(loc)upSt(i,"address",loc.address);}} style={{...IS,flex:1}}><option value="">—</option>{locations.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select>
